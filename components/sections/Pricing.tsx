  ...
  return (
    <SectionWrapper id="pricing">
      ...
      <motion.div
        ...
        className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:items-start"
      >
        {pricingPlans.map((plan) => (
          <motion.div
            ...
            className={`relative flex flex-col rounded-3xl p-6 transition-all duration-500 ${
              plan.highlighted
                ? "bg-gradient-to-b from-accent-maroon to-accent-brown text-white shadow-2xl ring-2 ring-accent-maroon/40 lg:scale-[1.04] lg:-translate-y-2"
                : "card-premium"
            }`}
          >
            {/* Badge */}
            {plan.badge && (
              <span
                className={`absolute -top-3 left-1/2 -translate-x-1/2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-widest shadow-md ${
                  plan.highlighted
                    ? "bg-amber-400 text-neutral-dark"
                    : "bg-accent-maroon text-white"
                }`}
              >
                {plan.highlighted ? <Sparkles size={10} /> : <Zap size={10} />}
                {plan.badge}
              </span>
            )}
